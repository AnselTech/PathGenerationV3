...
class RollerCommand(CustomCommand):
    ...
    def __init__(self, program, nextCustomCommand = None, rollerSpeed = 0):

        icon = graphics.getImage("Images/Commands/roller.png", 0.07)
        super().__init__(self.commandColors, program, icon, nextCustomCommand)

        self.slider = CommandSlider(self, -1, 1, 0.05, "Roller speed", rollerSpeed, dx = -50)
    ...
    def getCode(self) -> str:
        if self.slider.getValue() == 0:
            ...
            return f"\nrobot.roller->move_velocity({self.slider.getValue() * 100});\n"
